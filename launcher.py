...
def _check_node(ip: str, ssh_user: str, hostname: str) -> dict:
    ...
    checks["os"] = f"ok ({out})" if out in ("22.04", "24.04") \
        else f"FAIL — got {out}, need 22.04 or 24.04"

    out = run("free -m | awk '/^Mem:/{print $2}'")
    try:
        ram_mb = int(out)
        checks["ram"] = f"ok ({ram_mb} MB)" if ram_mb >= 1800 \
    ...
    try:
        disk_mb = int(out)
        checks["disk"] = f"ok ({disk_mb} MB free)" if disk_mb >= 20000 \
            else f"FAIL — {disk_mb} MB free, need >= 20000 MB"
    except ValueError:
        checks["disk"] = f"FAIL — could not parse: {out}"

    out = run("swapon --show")
    checks["swap"] = "ok (off)" if out == "" \
        else f"FAIL — swap is on: {out}"

    out = run("python3 --version")
    checks["python3"] = f"ok ({out})" if out.startswith("Python 3") \
        else f"FAIL — {out or 'not found'}"

    out = run("test -f /etc/kubernetes/admin.conf && echo exists || echo absent")
    checks["kubeadm_state"] = "ok (clean)" if out == "absent" \
